    ...
    def attack(self, audio, lengths, target, finetune=None):
        ...
        for i in range(MAX):
            ...
            for ii in range(self.batch_size):
                ...
                audio_process = audio[ii]
                audio_process = audio_process.astype('float64')
                th, psd_max = generate_mask.generate_th(audio_process, 16000)
                th = th.astype(int)
                print ("self.run.rescale")
                print (sess.run(self.rescale))

                if (self.loss_fn == "CTC" and i%10 == 0 and res[ii] == "".join([toks[x] for x in target[ii]])) \
                   or (i == MAX-1 and final_deltas[ii] is None):
                    # Get the current constant
                    rescale = sess.run(self.rescale)
                    print ("RESCALE RESCALE RESCALE")
                    print (rescale[ii])
                    if rescale[ii]*2000 > min ( np.max(np.abs(d)), np.max(np.abs(th))) :
                        ...
                        print ("****************")
                        print (d[ii])
                        rescale[ii] = min (np.max(np.abs(d[ii])) , np.max(np.abs(th)))/2000.0
